...
G33 - removed
G34 - Autolevel bed
G40 - X axis calibration
G41 - Y calibration (?)
G43 - Z height calibration. followed by left_test_print_code() and right_test_print_code() (lines 9689 and 9814)
...
{
	float res =0.125; //resoluci� d' 1/8 de volta
...
// Dual extruder X Calibration (commented and modified for ceramic printer by LH)
inline void gcode_G40(){
	...
	// purge filament
	current_position[E_AXIS] += 3; // 15; (LH, 17/11/2017)
	...
	// // retract after purge (commented out (LH, 08/11/2017))
	...
	// Y start/end coordinates of lines printed with left/right extruders
	...
	// print calibration lines and perimeter with LEFT extruder
	for (int i=0; i<NUM_LINES; i++)
	{
		// first print outer perimeter
		if (i == 0){
			...
			current_position[Z_AXIS] = 0.4; // 0.2; (LH, 20/11/2017)
			plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], 200, active_extruder);
			st_synchronize();
			if(gif_processing_state == PROCESSING_ERROR) return;
			
			// (commented out (LH, 20/11/20))
			...
			// // retract (commented out (LH, 08/11/2017))
			// current_position[E_AXIS] -= 4;
		...
		// // extrude to counteract retract (commented out (LH, 08/11/2017))
		// current_position[E_AXIS] += 4.1;
	...
	///////////////////////////////////////////////////
	/*** Change to RIGHT extruder (LH, 08/11/2017) ***/
	///////////////////////////////////////////////////

	// // commented out by LH (09/11/2017)
	// home_axis_from_code(true, true, false);
	// changeToolSigma(1);
	// axis_is_at_home(X_AXIS);
	gcode_T0_T1_auto(1);
	...
	// Print calibration lines and perimeter with RIGHT extruder
	for (int i=0; i<(NUM_LINES);i++)
	{
	...
	home_axis_from_code(true, true, false);
	if(gif_processing_state == PROCESSING_ERROR) return;
	changeTool(0); // switch back to left extruder
...
inline void gcode_G41(){
	...
	//Purge & up
	current_position[E_AXIS] += 3; // 15; (LH, 20/11/2017)
	plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], INSERT_SLOW_SPEED/60 , active_extruder);
	st_synchronize();
	if(gif_processing_state == PROCESSING_ERROR)return;
	...
	for (int i=0; i<(NUM_LINES);i++) //4 times
	{
		if (i == 0){
			...
			plan_buffer_line(current_position[X_AXIS],current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS],200, active_extruder);
			// current_position[E_AXIS]+=4.1; (LH, 20/11/2017)
			plan_buffer_line(current_position[X_AXIS],current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], INSERT_FAST_SPEED/60 , active_extruder);
			st_synchronize();
			if(gif_processing_state == PROCESSING_ERROR)return;
...
inline void gcode_G33(){
	...
	//Es calcula el pla a partir del pt2 i per tant �s l'origen (0,0,0)
	
	//Posicions relatives dels cargols de regulaci� respecte l'origen
	...
	//C�lcul dels vectors normalitzats de l'origen fins al cargol de regulaci�
	...
	//C�lcul de l'al�ada Z dels cargols de regulaci�
	...
	//Posici� relativa del centre de la plataforma respecte l'origen
	...
	//C�lcul de l'al�ada Z del centre de la plataforma
	float zc=(-planeNormal.x*centre.x-planeNormal.y*centre.y)/planeNormal.z;
	
	//C�lcul al�ades relatives cargols regulaci� respecte al centre (objectiu de regulaci�)
	float dz1 = zc-z1;
	float dz2 = zc-z2;
	float dz3 = zc-z3;
...
inline void gcode_G34(){
	...
	Serial.println(current_position[X_AXIS]);
	plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
	current_position[X_AXIS] -= 0; // 13; (LH, 8/11/2017)
	...
	current_position[X_AXIS] = x_home_pos(active_extruder) - 2;// (EB, 23/6/2017) +10;
	plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], 200, LEFT_EXTRUDER);
	st_synchronize();
	if(gif_processing_state == PROCESSING_ERROR) return;
	...
	current_position[X_AXIS]=x_home_pos(active_extruder)-2; // (LH, 18/10/2017) 10;
	plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], 200, RIGHT_EXTRUDER);
	...
	// Update Z Offset. We have to take into account the 2 different probe offsets
...
inline void gcode_T0_T1(){
	tmp_extruder = code_value();
	time_inactive_extruder[active_extruder] = 0;
	// check extruder number is valid
	if(tmp_extruder >= EXTRUDERS) {
		SERIAL_ECHO_START;
		SERIAL_ECHO("T");
...
void gcode_T0_T1_auto(int code){
	...
	// check requested extruder is valid
	if(tmp_extruder >= EXTRUDERS) {
		SERIAL_ECHO_START;
		SERIAL_ECHO("T");
		SERIAL_ECHO(tmp_extruder);
...
void process_commands()
{
	...
	if(code_seen('G'))
	{
		switch((int)code_value())
		{
			...
			case 33: // G33 Calibration Wizard by Eric Pallar�s & Jordi Calduch for RepRapBCN
			gcode_G33();
			break;
...
void left_test_print_code(){
	...
	// *modified extruder purge and retracts to suit current printer (LH, 17/11/2017)
	doblocking = true;
	if (active_extruder != LEFT_EXTRUDER) changeTool(LEFT_EXTRUDER);
	
	// LH*
	// current_position[E_AXIS] += 2; // 15; (LH*)
	// plan_buffer_line(current_position[X_AXIS],current_position[Y_AXIS],current_position[Z_AXIS],current_position[E_AXIS],INSERT_SLOW_SPEED/60,active_extruder);
	// st_synchronize();
	// if(gif_processing_state == PROCESSING_ERROR)return;
...
void right_test_print_code(){
	...
	// LH*
	// current_position[E_AXIS]-=4; //0.5 + 0.15 per ajustar una bona al�ada
	// plan_buffer_line(current_position[X_AXIS],current_position[Y_AXIS],current_position[Z_AXIS],current_position[E_AXIS],INSERT_FAST_SPEED/60 ,active_extruder);
	// st_synchronize();
	if(gif_processing_state == PROCESSING_ERROR)return;
	...
	for(int i = 1; i<=5;i++){
		...
		current_position[Y_AXIS] = initial_y_pos-distance_y;
		current_position[E_AXIS]+=extrusion_multiplier(distance_y);
		plan_buffer_line(current_position[X_AXIS],current_position[Y_AXIS],current_position[Z_AXIS],current_position[E_AXIS],40,active_extruder);
		st_synchronize();
		if(gif_processing_state == PROCESSING_ERROR)return;
